...
#sort_scores = sorted(df_sorted['scores'])
#df_sorted[df_sorted['log_scores']>20]
#reducing very large values for figure
#for i in range(len(df_sorted)):
#    if df_sorted.loc[i,'log_scores']>20:
#        df_sorted.loc[i,'log_scores'] = df_sorted.loc[i,'log_scores'] - 130
...
#df_rank_latency = df_rank_latency.sort_values('chi').reset_index(drop=True)
...
df_sorted['baseline'] = 3.5
import matplotlib.dates as mdates
axs[0].plot('DateTime','log_scores',data=df_sorted.loc[688:],marker='x', markerfacecolor='black', color='red', linewidth=1 )
axs[0].plot('DateTime','baseline', data=df_sorted.loc[688:],color='blue')

axs[1].plot('DateTime','battsV',data = df_sorted.loc[688:],marker='.', markerfacecolor='black', color='green', linewidth=1)
plt.show()

plt.savefig('batts_norm_anom.png')
"""
...
#labels = np.array(labels.replace(to_replace=-1,value=0))
#scores = np.array(scores)

#print('AE Test AUC = {}'.format(roc_auc_score(labels, scores)))
#print('AE Average Precision = {}'.format(average_precision_score(labels,scores)))

#k = evaluate_rate(classify_pts(labels,scores))
#print('AE results opt thres fpr:{:.2f}%, tpr:{:.2f}%, acc:{:.2f}%, prec:{:.2f}%, f1:{:.2f}%'.format(
#    100 * k[0], 100 * k[1], 100 * k[2], 100 * k[3], 100 * k[4]))
